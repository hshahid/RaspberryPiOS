//the operating system's text terminal
.section .data
.align 4

//address of the first valid character
terminalStart:
	.int terminalBuffer


//address of the last valid character
terminalStop:
	.int terminalBuffer

//address of the first displayed character
terminalView:
	.int terminalBuffer
	

//address of the first character of the text input
terminalColour:
	.byte 0xf

//text is stored for the console
.align 8
terminalBuffer:
	.rept 128*128
	.byte 0x7f
...
//the text last rendered to the screen by the console
terminalScreen:
	.rept 1024/8 * 768/16
	.byte 0x7f
	.byte 0x0	
...
/*set forcolor to the specified terminal. The low 4 bits of r0
contain the terminal colour code*/
TerminalColour:
	teq r0,#6
	ldreq r0,=0x02B5
	beq SetForeColour
...
//copy currently the part of TerminalBuffer that is currently displayed to the screen
...
/*Print a string to the terminal at current location. r0 contains a 
pointer to the ASCII encoded string, and r1 contains its length.*/
.globl Print
Print:
	teq r1,#0
	moveq pc,lr
...
//read the string input by the user and store it. Input after the specified maxlength is ignored.
